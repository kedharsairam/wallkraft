...
- Release pipeline (done):
  - `v*` tags are shared by two independent version tracks (Android 1.x, Windows 0.x); each platform's release workflow now has a `check` job that only proceeds when the tag matches its own version (`versionName` in build.gradle.kts / `version` in Cargo.toml), so a Windows v0.1.0 tag doesn't attach a stale APK and an Android v1.9.0 tag doesn't build a Windows installer
- winget publish (done):
